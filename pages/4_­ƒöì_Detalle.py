...
from utils import ensure_data_loaded, sidebar_filters, fmt_int

st.set_page_config(page_title="Detalle", page_icon="🔍", layout="wide")
...
# --- Tabla completa filtrable + export -------------------------------------
with tab_tabla:
...
with tab_pivot:
    ...
    if fila == colu:
        ...
        d = df.copy()
        if dims[fila] == "Periodo":
            d["Filas"] = d["Periodo"].dt.strftime("%Y-%m")
        else:
            d["Filas"] = d[dims[fila]]
        if dims[colu] == "Periodo":
            d["Columnas"] = d["Periodo"].dt.strftime("%Y-%m")
        else:
            d["Columnas"] = d[dims[colu]]
...
# --- Chequeo de calidad de datos contra tablas auxiliares -------------------
with tab_calidad:
    st.markdown(
